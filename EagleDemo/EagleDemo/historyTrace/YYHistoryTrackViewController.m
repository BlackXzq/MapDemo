...
//    NSArray *rightButtons = [NSArray arrayWithObjects:self.playButton, self.paramSetButton, self.refreshButton, nil];
    NSArray *rightButtons = [NSArray arrayWithObjects:self.paramSetButton, nil];
...
//        UIImage *setupIcon = [UIImage imageNamed:@"settings"];
        _paramSetButton = [[UIBarButtonItem alloc] initWithTitle:@"设置参数" style:UIBarButtonItemStylePlain target:self action:@selector(showParamSettings)];
...
-(UISegmentedControl *)wormSegmentControl {
    if (_wormSegmentControl == nil) {
        ...
        _wormSegmentControl.hidden = true;
    }
