...
-(UIBarButtonItem *)saveParamButton {
    if (_saveParamButton == nil) {
        _saveParamButton = [[UIBarButtonItem alloc] initWithTitle:@"查询" style:UIBarButtonItemStylePlain target:self action:@selector(saveParam)];
    }
...
-(UITextField *)entityNameTextField {
    if (_entityNameTextField == nil) {
        ...
        if (nil == entityNameStored || entityNameStored.length == 0) {
            ...
            _entityNameTextField.text = USERNAME;
        } else {
